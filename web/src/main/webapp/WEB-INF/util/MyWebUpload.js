(function ($, window) {
    ...
    function initWebUpload(item, options) {
        ...
        if (opts.auto) {
            ...
            uploader.on('fileQueued', function (file) {
                ...
                $list.append('<div id="' + $(item)[0].id + file.id + '" class="item">' +
                    ...
                    '<div class="webuploadDelbtn"><button type="button" class="btn btn-danger">删除</button></div><br />' +
                    '</div>');
                uploader.upload();
            });
        } else {
            uploader.on('fileQueued', function (file) {//队列事件
                $list.append('<div id="' + $(item)[0].id + file.id + '" class="item">' +
                    ...
                    '<div class="webuploadDelbtn"><font color="red">删除</font></div><br />' +
                    '</div>');
            });
        }
        ...
        uploader.on('uploadSuccess', function (file, response) {//上传成功事件
            ...
            console.log($(item)[0].id + file.id);
            //上传出错
            if (!response.result) {
                target.find('#' + $(item)[0].id + file.id).find('span.webuploadstate').html("上传出错");
            } else {
                target.find('#' + $(item)[0].id + file.id).find('span.webuploadstate').html('            <font color="green">已上传</font>');
                //拼接图片的路径年/月/文件名
                $hiddenInput.append('<input type="text" id="hiddenInput' + $(item)[0].id + file.id + '" class="hiddenInput" value="' + response.relativePath + '" />')

                var sysFile = response.relativePath;
                var dataItems = '<div id="' + $(item)[0].id + file.id + '_forDelete">';
                dataItems += '<input type="hidden" name="fileName" id="fileName" value="' + file.name + '"/>';
                dataItems += '<input type="hidden" name="sysFile" id="sysFile" value="' + sysFile + '"/>';
                dataItems += '</div>';

                $("#files").append(dataItems);
        ...
        uploader.on('fileDequeued', function (file) {
            debugger
            console.log("#hiddenInput" + $(item)[0].id + file.id);
            var sysFileName = $("#hiddenInput" + $(item)[0].id + file.id).val();
            console.log(sysFileName);
            if (sysFileName != null) {
                $.post(webuploaderoptions.deleteServer, {sysFileName: sysFileName}, function (data) {
                    //不再打印提示
                })
            }
            ...
            //删除准备上传的文件信息
            var toRemove = $("#" + $(item)[0].id + file.id + "_forDelete");
            $(toRemove).remove();
    ...
    $.fn.powerWebUpload = function (options) {
        ...
        if (typeof WebUploader == 'undefined') {
            var casspath = applicationPath + "/lib/webuploader/0.1.5/webuploader.css";
            $("<link>").attr({rel: "stylesheet", type: "text/css", href: casspath}).appendTo("head");
            var jspath = applicationPath + "/lib/webuploader/0.1.5/webuploader.js";
